...
module.exports = function auth(app){
    return function * () {
        var connection = new sql.Connection(app.conf.db.sqlserver);
        ...
        if (recordset.length === 1) {
            var token = jwt.sign({userId: recordset[0].UserId, userName: recordset[0].UserName}, app.conf.secure.authSecret);
